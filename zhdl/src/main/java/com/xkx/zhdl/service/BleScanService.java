...
public class BleScanService extends Service {
     ...
     */
    private Boolean fetchMultiBeacon = false;
    ...
    private int getRssiWithFilter(ArrayList<Integer> list, FilterMode mode) {
        ...
        if (mode == FilterMode.MODE_AVER) {// 平均值
            ...
            int size = list.size();
            if (size % 2 == 0) {
                rssi = Math.round((list.get(size / 2 - 1) + list.get(size / 2)) / 2);
            } else {
    ...
    private void setBeaconManager() {
        ...
        // SCAN_WAIT_MILLIS);
        brtBeaconMgr.setForegroundScanPeriod(SCAN_PERIOD_MILLIS, SCAN_WAIT_MILLIS);
        ...
        brtBeaconMgr.setRangingListener(new RangingListener() {
            ...
            public void onBeaconsDiscovered(RangingResult result) {
                if (fetchMultiBeacon == null) {
                    throw new IllegalStateException("setTriggerMode() must be called after service connection established.");
                }
                if (onBleScanListener == null) {
                    throw new IllegalStateException("BleScanListener must be set after service connection established.");
                }
                if (fetchMultiBeacon) {
    ...
    private void filterProcess(RangingResult result) {
        ...
        for (int i = 0; i < result.beacons.size(); i++) {
            ...
            if (everyRssimap.containsKey(beacon.macAddress)) {
                ...
                if (Math.abs(lastRssiMap.get(beacon.macAddress) - beacon.rssi) <= 5) {
                    ArrayList<Integer> list = everyRssimap.get(beacon.macAddress);
        ...
        if (loopCount % TIMES_FOR_LOOP == 0) {
            ...
            for (int i = 0; i < beacons.size(); i++) {
                ...
                brtBeacon.rssi = getRssiWithFilter(everyRssimap.get(brtBeacon.macAddress), FilterMode.MODE_AVER);
            }
            // 响应事件
            if (beacons.size() >= 1) {
    ...
    private void kalmanFilter(List<BRTBeacon> beacons) {
        ...
        for (int i = 0; i < beacons.size(); i++) {
            ...
            if (kalmanMap.containsKey(brtBeacon.macAddress)) {
                ArrayList<Integer> list = everyRssimap.get(brtBeacon.macAddress);
                if (list.size() >= SAMPLE_SIZE) {
                    list.remove(0);
                }
                ...
                // 卡尔曼增益
                double kalmanGain = Math.sqrt(kalman.devEsti / (kalman.devEsti + kalman.devMeas));
                // 最优值
                kalman.valOpti = kalman.valEsti + kalmanGain * (kalman.valMeas - kalman.valEsti);
            ...
            if (brtBeacon.macAddress.equals("EC:98:14:03:87:52")) {
                Kalman kalman = kalmanMap.get(brtBeacon.macAddress);
                Log.e("kalman", "测量:" + kalman.valMeas + ",估计:" + kalman.valEsti + ",最优:" + kalman.valOpti
                        // + "估值偏差:" + kalman.devEsti
                        + ",最优偏差:" + kalman.devOpti);
            }
        }
        if (loopCount % TIMES_FOR_LOOP == 0) {
            if (beacons.size() >= 1) {
                ...
                if (freshBeacon == null || !firstBeacon.equals(freshBeacon)) {
                    onBleScanListener.onNearBleChanged(freshBeacon, firstBeacon);
                }
    ...
    public interface OnBleScanListener {
         ...
         *
         * @param oriBeacon 原来的最近的beacon
         * @param desBeacon 现在的最近的beacon
         */
         ...
         * 周期性扫描回调方法
         *
         * @param scanResultList 扫描到的beacon列表
         */
    ...
    class BeaconData implements Comparable<BeaconData> {
        ...
        public boolean equals(Object obj) {
            ...
            if (beacon == null) {
                if (other.beacon != null) return false;
            } else if (!beacon.equals(other.beacon)) return false;
            return true;
        }
        ...
        public int compareTo(BeaconData another) {
            return (another.sumRssi / another.selfScanCount) - (this.sumRssi / this.selfScanCount);
        }
    }
